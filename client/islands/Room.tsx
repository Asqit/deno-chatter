...
export default function Room(props: RoomProps) {
	...
	useEffect(() => {
		...
		const ws = new WebSocket(`${location}/${isJoin ? "join" : "create"}/${roomKey}/username/${username}`);
	...
	return (
		<section className={"flex w-full h-screen"}>
			<div className={"w-1/3 border-r border-slate-300 p-8 max-w-md dark:bg-slate-900 dark:border-slate-500"}>
			...
			<div className={"flex-grow flex flex-col p-8"}>
				<ul className={"flex-grow flex flex-col items-start justify-start gap-4 overflow-y-auto"}>
					{messages.map((message, index) => {
						...
						if (user === username) {
							return (
								<li className={"bg-slate-500 text-white rounded-xl p-2 inline-block self-end justify-self-end"}>
									{msg}
								</li>
							);
						}
						...
						return (
							<li className={"bg-emerald-500 text-white dark:text-black rounded-xl p-2 inline-block"} key={index}>
